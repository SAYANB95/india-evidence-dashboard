...
export default function EditorialClient({records}:{records:EditorialRecord[]}){
  ...
  return <section className="editorial-queue" id="queue"><div className="editorial-head"><div><p className="eyebrow">Evidence migration queue</p><h2>Review the evidence before the interface.</h2></div><p>This queue audits database readiness only. “Ready for import” does not mean the claim has passed the protected two-person publication workflow.</p></div>
    ...
    <div className="editorial-count"><b>{filtered.length}</b> of {records.length} source-checked records <span>Read-only preview</span></div>
    <div className="editorial-table" aria-label="Editorial migration queue"><div className="editorial-table-head"><span>Record</span><span>Workflow</span><span>Evidence</span><span>Gaps & source</span></div>{filtered.map(record=><article key={record.id}>
      ...
      <div><b data-workflow={record.workflow}>{record.workflow}</b><small>{record.workflow === "Ready for import" ? "Required fields pass migration check" : "Human review required"}</small></div>
      ...
      <div><div className="gap-tags">{record.missingFields.length ? record.missingFields.map(item=><span key={item}>{item}</span>) : <b>No required-field gaps</b>}</div><small>{record.sourcePeriod}</small><a href={record.sourceUrl} target="_blank" rel="noreferrer">{record.sourceLabel} ↗</a></div>
    </article>)}</div>
