...
Version 1.9 additionally loads the official Health Dynamics of India 2022-23 state/UT bed table as on 31 March 2023. PHC, CHC, sub-district hospital, district hospital and medical-college categories remain separate, source `NA` remains null, and the 36 jurisdiction rows must reconcile to the published 8,18,661 India total. These records describe reported public-system capacity, not private-sector capacity, current vacancy or real-time emergency availability.

Version 2.0 additionally loads the official NHM MIS operational ambulance table as on 30 June 2024. ALS, BLS, patient-transport, boat, bike and other categories remain separate; every jurisdiction total must equal its category sum and all 36 rows must reconcile to 28,830. These figures describe vehicles reported available and operational under NHM, not live dispatch availability, GPS location, response time, or complete public/private ambulance supply.

The history page separates sources that must never be conflated. Its 29 individually reviewed biography cards and six reviewed event records are an expanding register, not state totals or a claim of completeness. A four-source map distinguishes the roughly 13,500-person Dictionary of Martyrs, the mixed-format Digital District Repository, Swatantrata Sainik Samman recognition records and the Cellular Jail aggregate. A Ministry of Culture Rajya Sabha answer dated 13 February 2025 supplies a complete published Cellular Jail table for 585 revolutionaries jailed during 1909–1938, including 398 from undivided Bengal. Historical Bengal is not equivalent to present-day West Bengal, and none of these sources alone is a census of every freedom-struggle participant.

Version 2.3 adds `/independence-day`, a responsive 15 August 2026 evidence edition. It combines national 2023 NCRB figures with a live-fetched, cached all-36-jurisdiction table for crimes against women, rape, girl-child POCSO cases and statutory SC/ST atrocities; national NFHS-5 survey context; and UDISE+ 2025-26 education indicators. It must label police registrations, survey prevalence and administrative education measures separately. It also shows the nine complaints reported after revamped SHe-Box complaint registration went live from 19 October to 29 November 2024 only as an early portal count—not as all workplace POSH complaints. Because no complete current public aggregate covers every employer Internal Committee and Local Committee, that national ICC total remains an explicit publication gap.

The homepage public-traffic strip is a real database-backed anonymous browser counter. Active means seen in the last five minutes; total begins at feature launch. It stores a one-way hash of a random first-party cookie and first/last-seen timestamps, excludes common crawlers, and stores no raw IP address, user-agent string, name or account ID. It is not a verified people count or a reconstruction of historic traffic.

Version 2.2 is an attractive responsive website foundation. Its first screen is the India civic-evidence statement, followed immediately by source-linked national GDP and state/UT output, per-capita NSDP coverage, a defined Central Government liabilities-to-reported-assets comparison, and clearly labelled research estimates of wealth distribution. An `All data` menu routes to the full product. `/catalog` defines 46 civic-evidence domains and exposes their source, cadence, geography, limitation and coverage state; the same ledger appears on every state/UT record. `/state-packs` adds comparable official UDISE+ 2023-24 education and SRS 2023 vital-statistics records for all 36 jurisdictions, with a source-labelled CSV export. `/history` includes four document-first dossiers, 29 reviewed profiles, six reviewed event records, a four-source register map, five public primary-document records and a 25-part ingestion register while explicitly separating facts, attributed interpretations and gaps. `/operations` exposes public-safe coverage and source-health state. `/corrections` stores privacy-minimised, rate-limited evidence correction requests without contact information. The release also contains verified national snapshots, one official CPCB station feed connected through data.gov.in, an all-36-jurisdiction official annual NCRB/ADSI matrix through data.gov.in, one working non-government public weather-model feed, a transport evidence room, a manually verified toll-plaza/project registry, a public read-only editorial console, a protected review manager, and a privacy-safe loans and schemes navigator. Managed Postgres stores reviewed evidence, audit records, corrections, anonymous action-rate records and source-health checks. Security headers, same-origin role-gated writes, request-rate records, CSV injection protection and SSRF-safe source checks are active. Public uploads and moderation remain disconnected. It must not be described as a complete historical dataset, district freedom-fighter register, health-service availability system or complete scheme directory.
...
- Loaded UDISE+ 2023-24 education and SRS 2023 vital-statistics records for all 36 jurisdictions; health, budget, inflation, environment, scheme and promise observations retain visible source-mapped gaps where comparable data has not been reviewed.
...
- `/api/system/status` exposes only public coverage and source-health summaries; it omits provider, authentication and database connection details.
...
## Operational and data-expansion queue

The agreed top-level product scope has no uncatalogued category remaining. The following work deepens evidence, operations or controlled capabilities; it does not justify fabricated completeness.

1. Replace the claimed identity development instance with production keys, then assign the first named editor/reviewer/publisher accounts and test the two-person publication path with real editorial users.
2. Establish an immutable external source archive; the stored correction workflow is now live.
