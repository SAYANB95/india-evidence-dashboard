...
The history page also separates two different grains that must never be conflated. Its 12 individually reviewed biography cards are a profile sample, not state totals. A Ministry of Culture Rajya Sabha answer dated 13 February 2025 supplies a complete published Cellular Jail table for 585 revolutionaries jailed during 1909–1938, including 398 from undivided Bengal. Historical Bengal is not equivalent to present-day West Bengal, and the Cellular Jail register is not a census of every freedom-struggle participant.

Version 1.9 is an attractive responsive website foundation. Its first screen is the India civic-evidence statement, followed immediately by source-linked national GDP and state/UT output, per-capita NSDP coverage, a defined Central Government liabilities-to-reported-assets comparison, and clearly labelled research estimates of wealth distribution. An `All data` menu routes to the full product. `/catalog` defines 46 civic-evidence domains and exposes their source, cadence, geography, limitation and coverage state; the same ledger appears on every state/UT record. `/state-packs` adds comparable official UDISE+ 2023-24 education and SRS 2023 vital-statistics records for all 36 jurisdictions, with a source-labelled CSV export. `/history` includes four document-first dossiers, a searchable 12-person reviewed seed, five public primary-document records and a 25-part ingestion register while explicitly separating facts, attributed interpretations and gaps. `/operations` exposes public-safe coverage and source-health state. `/corrections` stores privacy-minimised, rate-limited evidence correction requests without contact information. The release also contains verified national snapshots, one official CPCB station feed connected through data.gov.in, an all-36-jurisdiction official annual NCRB/ADSI matrix through data.gov.in, one working non-government public weather-model feed, a transport evidence room, a manually verified toll-plaza/project seed registry, a public read-only editorial console, a protected review manager, and a privacy-safe loans and schemes navigator. Managed Postgres stores reviewed seed evidence, audit records, corrections, anonymous action-rate records and source-health checks. Security headers, same-origin role-gated writes, request-rate records, CSV injection protection and SSRF-safe source checks are active. Public uploads and moderation remain disconnected. It must not be described as a complete historical dataset, district freedom-fighter register, health-service availability system or complete scheme directory.
